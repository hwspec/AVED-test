...
```
**chekcs current builds UUID**
```
grep -i "logic-uuid" hw/amd_v80_gen5x8_25.1/build/vivado.log
```
**Displays detailed PCIe link and device information (e.g., link speed, width, status) for the specified device in table format.**
```
ami_tool pcieinfo -d b1:00.0 -f table
```
**Reads and reports onboard hardware sensor data (e.g., temperature, voltage, power) for the specified device.**
```
ami_tool sensors -d b1:00.0
```
**Shows manufacturing information for the device, such as serial number, part number, firmware version, and other identification details.**
```
ami_tool mfg_info -d b1:00.0  
```
